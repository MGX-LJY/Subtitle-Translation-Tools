# -*- coding: utf-8 -*-
"""
AI SRT Translator v1.5  (2025‑07)
---------------------------------
• 并发翻译：默认 8 并发通道，显著提速
• 语气词规则：无法翻译的拟声词 → “啊～～”；若原文仅为单字“啊” → “啊～”
"""
...
def write_srt(path: Path, cues: List[SubtitleCue]):
    with path.open("w", encoding="utf-8") as f:
        for c in cues:
            f.write(f"{c.index}\n{c.start} --> {c.end}\n{c.translation or c.original}\n\n")

# --------------- Worker ----------------
class TranslateWorker(QThread):
    """后台线程：并发翻译 / 修复"""
    update_row = Signal(int,str,bool)
    token_inc  = Signal(int,int)
    progress   = Signal(int,int)
    finished   = Signal(str)
    error      = Signal(str)

    MAX_CONCURRENCY = 8          # 并发量，可根据速率限制调节
    ...
    def _extract(resp):
        ...
        if isinstance(resp,dict) and "choices" in resp:
            u=resp.get("usage",{}); return resp["choices"][0]["message"]["content"].strip(),u.get("prompt_tokens",0),u.get("completion_tokens",0)
        txt=str(resp).strip()
        if txt.lower().startswith("<!doctype") or "<html" in txt.lower():
            raise ValueError("返回 HTML — 检查 base_url/Token")
        return txt,0,0
    ...
    async def _main(self):
        ...
        for idx,c in enumerate(self.cues):
            if self.mode=="translate":
                tasks.append(self._translate_one(client,sem,idx,c))
            else:
                tasks.append(self._fix_one(client,sem,idx,c))
        total=len(tasks)
    ...
    async def _translate_one(self,client,sem,idx,cue):
        async with sem:
            ...
            cue.translation=txt; cue.fixed=False
            self._ptok+=pt; self._ctok+=ct
            self.update_row.emit(idx,txt,False)
    ...
    async def _fix_one(self,client,sem,idx,cue):
        async with sem:
            rsp=await client.chat.completions.create(
                ...
                messages=[
                    {"role":"system",
                     "content":"你是一名中文编辑助手。记住你修复的是对话，一定要是对话。孜然流畅"
                               "如果给定句子逻辑已通顺，判断范围宽泛一些，请只回复 SKIP。"
                               "否则输出修正文，不附加其它文本。"},
            ...
            fixed=False
            if txt.strip()!="SKIP":
                cue.translation=txt; fixed=True
            cue.fixed=fixed
            self._ptok+=pt; self._ctok+=ct
            self.update_row.emit(idx,cue.translation,fixed)
            self.token_inc.emit(pt,ct)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.cfg=Config(); self.cues=[]; self.hist=[]
        self._ptok=self._ctok=0
        self._ui()

    # ---- UI ----
    def _ui(self):
        self.setWindowTitle("AI 字幕翻译器"); self.resize(1100,650)
        ...
        for t,cb in (("打开",self.open_file),("保存",self.save_file),("导出",self.export_file),
            ...
            if t: tb.addAction(QAction(t,self,triggered=cb))
            else: tb.addSeparator()

        self.table=QTableWidget(0,7); self.table.setHorizontalHeaderLabels(["#","开始","结束","原文","译文","修复","恢复"])
    ...
    def open_file(self):
        ...
        if fn:
            self.cues=parse_srt(Path(fn)); self._refresh(); self.hist.clear()
    ...
    def _refresh(self):
        ...
        for r,c in enumerate(self.cues): self._set_row(r,c.translation,c.fixed)
    def _set_row(self,row,txt,fixed):
        cue=self.cues[row]; cue.translation=txt; cue.fixed=fixed
        for col,val in enumerate((cue.index,cue.start,cue.end,cue.original,cue.translation,"✓" if fixed else "", "")):
    ...
    def _restore(self,row):
        self.cues[row].translation=""; self.cues[row].fixed=False
        self._set_row(row,"",False); self.status.showMessage(f"第 {row+1} 行已恢复")
    ...
    def _launch(self,mode):
        ...
        self._ptok=self._ctok=0; self._upd_tok()
        ...
        self.worker.token_inc.connect(lambda p,c:(self._count_tok(p,c)))
    ...
    def _count_tok(self,p,c): self._ptok+=p; self._ctok+=c; self._upd_tok()
    def _upd_tok(self): self.token_lbl.setText(f"Tokens P:{self._ptok} | C:{self._ctok}")

    # ---- Undo ----
    def _push_hist(self): self.hist.append([SubtitleCue(**vars(c)) for c in self.cues])
    def undo(self):
        if self.hist:
            self.cues=self.hist.pop(); self._refresh(); self.status.showMessage("已撤销")
